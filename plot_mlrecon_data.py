...
def tags(file):
    return file.split("/")[-1][:-4].split("_")

def info(file):
    _tags = tags(file)
    return { "circuit" : _tags[0],
             "layers" : _tags[1],
             "frags" : _tags[2],
             "qubits" : _tags[3] }

def basename(circuit_info):
    return "_".join([ str(tag) for tag in circuit_info.values() ]) + f"_{orientation}"
...
circuit_types = set( info(file)["circuit"] for file in data_files )

for circuit_type in circuit_types:
    circuit_info = { "circuit" : circuit_type }

    circuit_layers = all_values("layers", **circuit_info)
    for layers in circuit_layers:
        circuit_info["layers"] = layers

        circuit_frags = all_values("frags", **circuit_info)
        if orientation == "horiz":
            fig_rows = 2
            fig_cols = len(circuit_frags)
            fig_width = 1.5 * fig_cols
            fig_height = 3.4
        ...
        for frag_idx, frags in enumerate(circuit_frags):
            ...
            def file(qubits):
                circuit_info["qubits"] = qubits
                files = select_files(**circuit_info)
                assert(len(files) == 1)
                return files[0]
            ...
            for fidelities, label, color, marker in zip(data.T, labels, colors, markers):
                infidelity_mean = 1 - np.mean(fidelities, axis = 0)
                ...
                ax_avg.semilogy(qubit_nums, infidelity_mean, marker, **plot_args)
                ax_std.semilogy(qubit_nums, fidelity_std, marker, **plot_args)

            del circuit_info["qubits"]
            del circuit_info["frags"]
        ...
        plt.savefig(basename(circuit_info) + ".pdf", bbox_inches = "tight")

        del circuit_info["layers"]
